...
// Declare openai client variable, but do not initialize it yet.
let openai: OpenAI | null = null;

/**
 * Lazily initializes and returns the OpenAI client instance.
 * This prevents API key errors on app load if the key is missing.
 */
 ...
 * This function constructs a prompt from user options, sends it to an OpenAI model,
...
export const generateCctvPrompt = async (options: GeneratorOptions): Promise<string> => {
  ...
  const client = getOpenAIClient(); // Get or initialize the client.
...
`;
  ...
  try {
    ...
    const promptText = completion.choices[0]?.message?.content;
    if (!promptText) {
