...
export const PromptGenerator: React.FC<PromptGeneratorProps> = ({ onGenerate, isLoading, usageLimitMessage, cooldownEndTime, onNavigateToAuth }) => {
    ...
    if (cooldownEndTime) {
      const updateTimer = () => {
        ...
        if (remaining > 0) {
          ...
          setTimeRemaining('');
          // Optionally, you could trigger a state refresh in the parent here
        }
      };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="pt-2">
        ...
        {isLimitReached && (
           <div className="mt-4 text-center text-sm text-yellow-400 bg-yellow-900/30 border border-yellow-400/20 p-3 rounded-sm">
            {timeRemaining ? (
              <span>{usageLimitMessage} Next generation in <strong>{timeRemaining}</strong>.</span>
            ) : usageLimitMessage?.includes('sign up') ? (
              <span>
                {usageLimitMessage}
                <button onClick={onNavigateToAuth} className="font-bold underline text-yellow-300 hover:text-white ml-2">
                    [ Sign Up Now ]
                </button>
              </span>
            ) : (
