...
import { getRecentPrompts, addPrompt, getUserUsageStats } from './services/supabaseService';
...
type Page = 'generator' | 'about' | 'contact' | 'terms' | 'privacy' | 'auth' | 'dashboard' | 'gallery' | 'blog';
type UserUsage = { count: number; cooldownEndTime: number | null };
...
const App: React.FC = () => {
  ...
  // Initialize anonymous usage from localStorage
  useEffect(() => {
    const savedUsage = localStorage.getItem('anonymousGenerations');
    setAnonymousUsage(savedUsage ? parseInt(savedUsage, 10) : 0);
  }, []);

  const fetchUserUsageStats = useCallback(async () => {
    if (!user) {
      setUserUsage(null);
      return;
    }
    const { promptCount, lastBatchStartTime } = await getUserUsageStats(user.id);
    let cooldownEndTime: number | null = null;
    if (promptCount >= USER_LIMIT && lastBatchStartTime) {
      cooldownEndTime = new Date(lastBatchStartTime).getTime() + COOLDOWN_PERIOD_HOURS * 60 * 60 * 1000;
    }
    setUserUsage({ count: promptCount, cooldownEndTime });
  }, [user]);
  ...
  }, [activePage, fetchRecentPrompts, fetchUserUsageStats]);
  ...
  // Check usage limits whenever usage state changes
  useEffect(() => {
    if (user) {
      if (userUsage) {
        if (userUsage.count >= USER_LIMIT && userUsage.cooldownEndTime && Date.now() < userUsage.cooldownEndTime) {
          setUsageLimitMessage("You've reached your generation limit.");
        } else {
          setUsageLimitMessage(null);
        }
      }
    } else {
      if (anonymousUsage >= ANONYMOUS_LIMIT) {
        setUsageLimitMessage(`You've used your ${anonymousUsage}/${ANONYMOUS_LIMIT} free generations. Please sign up to continue.`);
      } else {
        setUsageLimitMessage(null);
      }
    }
  }, [user, anonymousUsage, userUsage]);

  const handleGenerate = useCallback(async (options: GeneratorOptions) => {
    // Enforce usage limits before calling the API
    if (usageLimitMessage) {
        setError("Usage limit reached. Please check the message below the generate button.");
        return;
    }
    ...
    try {
      ...
      // For logged-in users, save the prompt automatically
      if (user) {
      ...
      if (user) {
        ...
        if (activePage === 'generator') fetchRecentPrompts();
      } else {
        const newCount = anonymousUsage + 1;
  ...
  }, [user, anonymousUsage, fetchUserUsageStats, usageLimitMessage, activePage, fetchRecentPrompts]);
  ...
  const renderPage = () => {
    switch (activePage) {
      ...
      case 'blog':
        return <BlogPage onBack={() => navigate('generator')} onNavigateToGenerator={scrollToGenerator} />;
      case 'generator':
      default:
        return (
            ...
            <section id="generator" ref={generatorRef} className="scroll-mt-24">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
                <PromptGenerator 
                  ...
                  cooldownEndTime={userUsage?.cooldownEndTime ?? null}
